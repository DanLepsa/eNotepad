...
import extendedContextMenu from "electron-context-menu";
...
app.on("open-file", async (event: Electron.Event, path: string) => {
  ...
  if (BrowserWindow.getAllWindows().length === 0) {
    ...
    await delay(500); /// waiting 500ms.
    BrowserWindow.getAllWindows()[0].webContents.send("FILE_OPEN", path);
  } else {
    BrowserWindow.getAllWindows()[0].webContents.send("FILE_OPEN", path);
  }
});
...
const setMainMenu = (mainWindow: BrowserWindow) => {
  ...
  extendedContextMenu({}); // allows for right click copy,cut,paste etc.
